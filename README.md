...
普通接头只允许两芯连接，接头边按所接电缆芯线的本地物理 A/B 标签自动判断是直通还是交换：
...
`A` 旁路和 `B` 旁路必须使用同一条电缆 pair，并终止到同一个封帽声明的 A/B 端。普通 degree-two splice 上挂出的封帽会失败；未终止于封帽的旁路也会失败。
...
YAML 语法、schema、未知字段、重复键、未知节点、端口重复声明等结构错误返回 HTTP `422`。拓扑冲突不会被伪装成结构错误，也不会出现任何可送话结论。
